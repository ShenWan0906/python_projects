...
def match_address(province, city, district, geo_index):
    ...
    # ✅ 情况2：region_name 为空，只匹配省市
    all_districts = geo_index[best_province][best_city]
    all_points = []
    for dist_list in all_districts.values():
        all_points.extend(dist_list)

    if not all_points:
        return None, {
            'fail_reason': f'省市匹配成功但无区县数据 | 省: {best_province} 市: {best_city}'
        }

    avg_score = (best_province_score + best_city_score) / 2
    return random.choice(all_points), {'score': avg_score}
